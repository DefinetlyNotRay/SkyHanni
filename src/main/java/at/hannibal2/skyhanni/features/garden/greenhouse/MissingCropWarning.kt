...
object MissingCropWarning {
    ...
    fun onInventoryFullyOpened(event: InventoryFullyOpenedEvent) {
        ...
        if (category == null || targetPosition == null) {
            ChatUtils.chat(
                "§cCould not save diagnosed crop. " +
                    "§7Menu crop: §e${category?.displayName ?: "unknown"}§7, " +
        ...
        val position = nearbyPosition ?: if (targetedCategory == null) {
            targetPosition.add(y = 1)
        } else {
            targetPosition
        }
        pendingDiagnostic = null
    ...
    fun onSecondPassed() {
        ...
        storage?.diagnosedCropPositionsByPlot?.let { saved ->
            for ((plotId, positions) in runtimeDiagnosedPositionsByPlot) {
        ...
        // Wait for multiple identical full scans so joining/loading block updates cannot cause a false warning.
        ...
        val allPresentCrops = presentAcrossGreenhouses + diagnosedPresentCrops(plot.id)

        val missing = CropCategory.entries.toSet() - allPresentCrops
    ...
    private fun scanGreenhouse(plot: GardenPlotApi.Plot): Set<CropCategory> {
    ...
    private fun diagnosedPresentCrops(currentPlotId: Int?): Set<CropCategory> =
        diagnosedPositionsByPlot().flatMap { (plotId, positions) ->
            ...
            // Other Greenhouses are trusted until the player enters them and a full scan settles.
            category.takeIf {
    ...
    private val deadCropBlocks = setOf(
        Blocks.DEAD_BUSH,
        Blocks.CHORUS_PLANT,
    ...
    private fun showDebugState() {
        ...
        val diagnosedPositions = diagnosedPositionsByPlot()
        val savedCrops = savedByPlot.values.flatten()
            .mapNotNullTo(mutableSetOf(), CropCategory::fromStorageName)
        val missing = CropCategory.entries.toSet() - savedCrops
        ...
        ChatUtils.chat(
            buildString {
                ...
                appendLine(" §7Checker considers inside: §e${isInGreenhouse()}")
                appendLine(" §7Current plot: §e${plot?.id ?: "none"}")
                appendLine(" §7Live scan: §e${liveCrops.namesOrNone()}")
                appendLine(" §7Saved plots: §e${savedByPlot.keys.sorted().joinToString().ifEmpty { "none" }}")
                appendLine(" §7Saved crops: §e${savedCrops.namesOrNone()}")
                appendLine(
                    " §7Diagnosed positions: §e" +
                        diagnosedPositions.entries.joinToString { (plotId, positions) ->
                            "$plotId=[${positions.keys.joinToString()}]"
                        }.ifEmpty { "none" },
                )
                val allPresent = savedCrops + diagnosedPresentCrops(plot?.id)
                append(" §7Missing across all plots: §e${(CropCategory.entries.toSet() - allPresent).namesOrNone()}")
            },
        )
    }
    ...
    private fun diagnosedPositionsByPlot(): Map<Int, Map<String, LorenzVec>> = buildMap {
        ...
        runtimeDiagnosedPositionsByPlot.forEach { (plotId, positions) ->
            val combined = get(plotId).orEmpty() + positions
            put(plotId, combined)
        }
    }
